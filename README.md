...
Notebook-generated datasets
- Notebook 04 can generate reusable scenario CSVs under `data/synthetic/` (e.g. monetization/activity/mixed scenarios).
- Notebooks 05–07 can load those CSVs if present, or fall back to generating synthetic data on the fly.

CLI output bundles (recommended workflow)
Most CLI commands write an output bundle directory via `--out`, containing:
- `results.json` (machine-readable)
- `report.md` (human-readable)
- `tables/*.csv` and `plots/*.png` (artifacts referenced from `results.json`)

In CI, example bundles are generated under `out/` (e.g. `out/run_mean`, `out/run_ratio`, `out/run_sequential_mean`, `out/run_sequential_ratio`).
