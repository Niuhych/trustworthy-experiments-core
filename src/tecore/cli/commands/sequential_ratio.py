...
def cmd_sequential_ratio(args) -> int:
    ...
    spec = SequentialSpec(
        ...
        num_col=str(getattr(args, "num")),
        den_col=str(getattr(args, "den")),
    ...
    baseline_mode = str(getattr(args, "baseline_mode", "first_look"))
    df_lin, r0, warn_lin = linearize_ratio_frame(df, spec, schedule, baseline_mode=baseline_mode)
    ...
    res.diagnostics = dict(res.diagnostics) | {"ratio_baseline_r0": float(r0), "baseline_mode": baseline_mode}
